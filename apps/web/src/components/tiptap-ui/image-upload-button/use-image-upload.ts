...
export function canInsertImage(editor: Editor | null): boolean {
  ...
  // Using editor.can().insertContent for block nodes can be overly strict
  // in common cursor positions (e.g., inside a paragraph). Since our custom
  // command replaces the current selection with the upload node, allow it
  // whenever the extension is available and the editor is editable.
  return true;
}
...
export function insertImage(editor: Editor | null): boolean {
  ...
  try {
    // Prefer the extension command which replaces the current selection
    // with the imageUpload node, making insertion work reliably in
    // paragraphs and other typical contexts.
    if (typeof (editor.commands as any).setImageUploadNode === "function") {
      return editor.chain().focus().setImageUploadNode().run();
    }

    // Fallback to direct insertContent if command is unavailable
