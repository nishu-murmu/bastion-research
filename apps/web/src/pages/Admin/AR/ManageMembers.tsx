...
import { Search, Plus, ChevronDown, Eye, Edit, Users, Building, Trash2, Filter, Settings } from 'lucide-react';
...
const MemberManagementDashboard = () => {
  ...
  const columns = [
    ...
    { key: 'profileDisplayName', label: 'Profile Display Name', width: '200px' },
  ...
  useEffect(() => {
    ...
    if (searchTerm) {
      filtered = filtered.filter(member =>
        ...
        member.profileDisplayName.toLowerCase().includes(searchTerm.toLowerCase())
      );
    }

    if (selectedPlan !== 'Select Plans') {
      filtered = filtered.filter(member => member.memberPlan === selectedPlan);
    }
  ...
  const PlanBadge = ({ plan }) => {
    ...
    return (
      <div className={`w-8 h-8 rounded-full ${getPlanColor(plan)} flex items-center justify-center text-white text-xs font-medium`}>
        {plan}
      </div>
    );
  };
  ...
  const RowActionMenu = ({ member }) => (
    <div className="absolute right-2 top-1/2 transform -translate-y-1/2 bg-blue-600 rounded-lg shadow-lg flex items-center space-x-1 px-2 py-1 z-10">
      <button className="p-1 text-white hover:bg-blue-700 rounded" title="View Details">
        <Eye size={16} />
      </button>
      <button className="p-1 text-white hover:bg-blue-700 rounded" title="Edit">
        <Edit size={16} />
      </button>
      <button className="p-1 text-white hover:bg-blue-700 rounded" title="Users">
        <Users size={16} />
      </button>
      <button className="p-1 text-white hover:bg-blue-700 rounded" title="Manage Plans">
        <Building size={16} />
      </button>
      <button className="p-1 text-white hover:bg-blue-700 rounded" title="Delete">
        <Trash2 size={16} />
      </button>
    </div>
  );
            ...
            <div className="relative">
              ...
              {showColumnOptions && (
                <div className="absolute right-0 top-full mt-2 w-64 bg-white border border-gray-200 rounded-lg shadow-lg z-20">
                  <div className="p-4">
                    <div className="text-sm font-medium text-gray-900 mb-3">Toggle Columns</div>
                    {columns.filter(col => col.key !== 'select').map(column => (
                      <label key={column.key} className="flex items-center space-x-2 mb-2">
                        <input
                          type="checkbox"
